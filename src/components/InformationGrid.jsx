...
const InformationGrid = ({id, name, email, country, birth_date}) => {
  return (
    <div >
      <li key={id} className="col-span-1 divide-y divide-violet-900 rounded-lg border-solid border-2 border-violet-900 bg-violet-200 shadow-2xl list-none">
        ...
        <div className="flex w-full items-center justify-between space-x-6 p-2">
          ...
          <img className="h-10 w-10 flex-shrink-0 rounded-full bg-gray-100" src={user} alt="user" />
            ...
            <span className="mt-3 mb-3 truncate text-sm inline-block flex-shrink-0 rounded-full bg-yellow-400 px-2 py-0.5 font-medium text-yellow-800">{email}</span>
          </div>
        </div>


        <div>
          <div className="-mt-px flex divide-x divide-violet-900">
